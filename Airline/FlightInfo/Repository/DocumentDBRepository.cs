    ...
    public static class DocumentDBRepository<T> where T : class
    {
        ...
        public static void DeleteDocumentAsync(string id)
        {
            var passengertodelete =
               client.CreateDocumentQuery<Cities>(ConfigurationManager.AppSettings["collection"])
                     .Where(d => d.CityCode == id)
                     .AsEnumerable()
                     .First();

            Document doc = GetDocument(client, passengertodelete.CityCode);
